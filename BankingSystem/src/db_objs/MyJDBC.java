...
//
            ...
            if (resultSet.next()) {
                ...
                // return user object
                return new User(userId, username, password, currentBalance, cardnumber);
                // mình sử dụng cái select để lấy ra
            }
//
    ...
    public static boolean register(String username, String password) {
        try {
            ...
            if (!checkUser(username)) {
                ...
                PreparedStatement preparedStatement = connection.prepareStatement(
                        "INSERT INTO users(username, password, current_balance, cardnumber) " +
                                "VALUES(?, ?, ?, ?)"
                );
...
////
    ...
    public static int generateUniqueCardNumber() {
        SecureRandom random = new SecureRandom();
        int cardnumber = random.nextInt(900000000) + 100000000;
        return cardnumber;
    }
...
////
    ...
    public static String getUsernameByCardNumber(String cardNumber) {
        // Implement the logic to retrieve the username based on the card number
        // This method should query the database or any other data source to find the associated username

        // Establish a connection to the database
        try {
            Connection connection = DriverManager.getConnection(DB_URL, DB_USERNAME, DB_PASSWORD);
            PreparedStatement updateBalance = connection.prepareStatement(
                    "SELECT username FROM users WHERE cardnumber = ?"
            );
            updateBalance.setString(1, cardNumber);
            ...
            if (resultSet.next()) {
                ...
                // If no record is found, return a default or error message
                return "Unknown user";
            }
    ...
    public static boolean transfer(User user, int transferredCardNumber, float transferAmount) {
        //when transfer need user to check user_id, and transferredCardnumber to input in the textfield and transferAmount to add in db
        try {
            Connection connection = DriverManager.getConnection(DB_URL, DB_USERNAME, DB_PASSWORD);
            ...
            //The ? is a placeholder for a parameter that will be substituted when the prepared statement is executed.

            queryUser.setInt(1, transferredCardNumber);
            ResultSet resultSet = queryUser.executeQuery();

            if (resultSet.next()) { //ban đầu là while
                int transferredUserId = resultSet.getInt("id");

                // Check if the initiating user's account ID is the same as the transferred user's account ID
                //delete this if means that user can transfer to their own account!!
                if (user.getId() == transferredUserId) {
                    // Display an error message, as the user is trying to transfer to their own account
//                    System.out.println("Cannot transfer money to your own account.");
//                    JOptionPane.showMessageDialog(this, "Please choose an account other than the source account");
                    return false;
                }
